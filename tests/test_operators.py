...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert 0 <= sigmoid(a) <= 1.0, 'It is always between 0.0 and 1.0.'
    assert abs(1 - sigmoid(a) - sigmoid(-a)) < 1e15, 'one minus sigmoid is the same as sigmoid of the negative'
    assert sigmoid(0) == 0.5, 'It crosses 0 at 0.5'
    assert sigmoid(a - 10) <= sigmoid(a) <= sigmoid(a + 10), 'It is  strictly increasing.'
...
def test_identity(x: float) -> None:
    """Test the identity property of multiplication (x * 1 = x)."""
    assert mul(x, 1.0) == x
    assert mul(1.0, x) == x
